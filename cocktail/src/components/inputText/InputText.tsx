...
export const InputText: React.FC<Props> = ({ onClose }) => {
  ...
  useEffect(() => {
    if (searchedDrink.length > 0) {
      searchDrink();
    }
  }, [searchedDrink]);

  const handleSearch = () => {
    navigate("/results", { state: { drinks } });
  };
    ...
    >
      <div
        ...
        className={`absolute flex h-14 w-80 rounded-full text-center outline-none items-center bg-white px-9 text-principalTitleColor transition-all duration-500 ${
          isMounted ? "-translate-y-20 opacity-100" : "translate-y-0 opacity-0"
        }`}
      >
        <input
          ref={inputRef}
        ...
        />
        <IoSearchOutline onClick={handleSearch} className="cursor-pointer" />
      </div>
    </div>
  );
};
